...
uint8_t wasRawIndex = 0;
uint16_t wasRawReadings[128];

uint16_t prevWasReading = 0;
float wasChangeRate = 0;
float wasErrorOverTime = 0;
...
elapsedMicros wasChangeRateElapsedTime;
...
void handleMotorControls(JsonDocument &document)
{
    ...
    else
    {
        ...
        sendMessageToEvents(message, "motor_enabled");
    }
...
void updateStepper()
{
    // wasRawReadings[wasRawIndex] = readADC();
    // wasRawIndex = (wasRawIndex + 1) % 128;
    // uint32_t newWasReading = 0;
    // for (int i = 0; i < 128; i++)
    // {
    //     newWasReading += wasRawReadings[i];
    // }
    // wasReading = newWasReading >> 7;
    wasReading = readADC();
    if (wasChangeRateElapsedTime > 1e4)
    {
        wasChangeRate = 0.9 * wasChangeRate + 0.1 * float(wasReading - prevWasReading) / float(wasChangeRateElapsedTime / 1e6);
        prevWasReading = wasReading;
        if (motorEnabled)
        {
            wasErrorOverTime = 0.99 * wasErrorOverTime + 0.01 * float(abs(wasTarget - wasReading));
        }
        wasChangeRateElapsedTime = 0;
    }
    ...
    if (stepperLastCommandElapsedTime > STEPPER_COMMAND_UPDATE_US)
    {
        if (motorEnabled)
        {
            ...
            sendMessageToEvents(message, "motor_no_command");
        }
        motorCalibration = false;
    }
    if (stepperElapsedTime > STEPPER_PERIOD_US)
    {
        ...
        if (motorEnabled)
        {
            ...
            if (abs(stepperRPMActual) > motorConfig.TCOOLTHRS_RPM && stepper.stallguard() && !motorStalled)
            {
                ...
                sendMessageToEvents(message, "motor_stalled");
                stallElapsedTime = 0;
                motorStalled = true;
            }
            else
            {
                ...
                if (motorCalibration)
                {
                    if (wasTarget != motorConfig.wasMin && wasTarget != motorConfig.wasMax)
                    {
                        wasTarget = motorConfig.wasMin;
                    }
                    int16_t difference = wasReading - wasTarget;
                    if (difference <= 0 && wasTarget == motorConfig.wasMin)
                    {
                        wasTarget = motorConfig.wasMax;
                    }
                    else if (difference >= 0 && wasTarget == motorConfig.wasMax)
                    {
                        wasTarget = motorConfig.wasMin;
                    }
                }
