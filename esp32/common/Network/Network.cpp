...
IPAddress udpReceiveAddress;
IPAddress udpSendAddress;
...
void startWiFiClient() {
  ...
  for (int i = 0; i < 5; i++) {
    if (strlen(wifiConfig.ssid[i]) > 0) {
      wifiMulti.addAP(wifiConfig.ssid[i], strlen(wifiConfig.password[i]) > 0
                                              ? wifiConfig.password[i]
                                              : NULL);
      Serial.printf("\t%s\n", wifiConfig.ssid[i]);
    }
  }
  ...
  for (int i = 0; i < foundNetworks; i++) {
    Serial.printf("\tSSID: %-32s | AUTH: %-15s| RSSI: %d\n",
                  WiFi.SSID(i).c_str(),
                  wifiAuthTypeToString(WiFi.encryptionType(i)), WiFi.RSSI(i));
  }
...
void setupUDP(AuPacketHandlerFunction onPacketCallback) {
  udpReceiveAddress.fromString(wifiConfig.multicastReceiveIp);
  udpSendAddress.fromString(wifiConfig.multicastSendIp);

  if (udp.listenMulticast(udpReceiveAddress, wifiConfig.udpReceivePort)) {
...
void checkWiFiStatus() {
  ...
  if (WiFi.getMode() == WIFI_MODE_STA) {
    ...
    if (status == WL_DISCONNECTED) {
      ...
      if (hardwareIdentifierSendTime > 10000) {
        Serial.printf("Sending identifier message to: %s:%d\n",
                      udpSendAddress.toString().c_str(),
                      wifiConfig.udpSendPort);
        sendUdpPacket("Steering hardware", 18, udpSendAddress,
                      wifiConfig.udpSendPort);
        hardwareIdentifierSendTime = 0;
      }
#endif
#ifdef AUTOSTEERING_REMOTE_CONTROL
      if (hardwareIdentifierSendTime > 2000) {
        Serial.printf("Sending identifier message to: %s:%d\n",
                      udpSendAddress.toString().c_str(),
                      wifiConfig.udpSendPort);
        sendUdpPacket("Remote control", 15, udpSendAddress,
                      wifiConfig.udpSendPort);
        hardwareIdentifierSendTime = 0;
      }
#endif
    ...
    if (!wifiConfig.startInAPMode && wifiConfig.hasKnownNetworks() &&
        connected < 1 && lastAPConnectedDeviceDisconnectTime > 5000) {
...
void sendUdpPacket(uint8_t *data, int packetSize, IPAddress destinationIP,
                   uint destinationPort) {
  if (udp.writeTo(data, packetSize, destinationIP, destinationPort)) {
#ifndef STEPPER_MOTOR_TESTING
...
// void sendUdpPacket(uint8_t *data, int packetSize, char *destinationHost,
//                    uint destinationPort) {
//   if (udp.writeTo(data, packetSize, destinationHost, destinationPort, )) {
// #ifndef STEPPER_MOTOR_TESTING
//     digitalWrite(SEND_LED_PIN, HIGH);
// #endif
//     sendLEDTime = 0;
...
void sendUdpPacket(const char *data, int packetSize, IPAddress destinationIP,
                   uint destinationPort) {
  if (udp.writeTo((const uint8_t *)data, packetSize, destinationIP,
                  destinationPort)) {
#ifndef STEPPER_MOTOR_TESTING
    digitalWrite(SEND_LED_PIN, HIGH);
#endif
...
void sendUdpData(uint8_t *buffer, int messageSize) {
  sendUdpPacket(buffer, messageSize, udpSendAddress, wifiConfig.udpSendPort);
}
